...
APP2_lowerthresblock::APP2_lowerthresblock(QGraphicsScene * parent, float thres){
    ...
    this->thres_label = new APP2_valuelabel("MIN", thres);

    QList<APP2_valuelabel *>vllist;
    vllist.push_back(this->thres_label);
    this->mommy_canvas->addItem(this->thres_label);

    this->mommy_canvas->addItem(this->simpleblock = new APP2_simpleblock(this, QRectF(0, 0, 100, 100), this->signalnodes, this->slotnodes, this->mommy_canvas->addText("Lower Thres"), vllist));
    this->mommy_canvas->addItem(this->resizenode = new APP2_resizenode);

    connect(this->simpleblock, &APP2_simpleblock::siSettingsMenuShow, this, &APP2_lowerthresblock::slSettingsMenuShow);
...
void APP2_lowerthresblock::slSettingsMenuShow(void){
    ...
    connect(button_ok, &QPushButton::clicked, this, &APP2_lowerthresblock::slSetValues);
...
void APP2_lowerthresblock::ProcessBlockData(void){
    ...
    qDebug() << "LOWER THRES BLOCK Signal " << this->output->current_value;
}
